...
static void add_cpus_and_nic(sg4::NetZone* node_zone, bool with_gpus, bool with_nvme)
{
  ...
  for (unsigned int c = 0; c < 2; c++) {
    ...
    node_zone->add_route(cpus[c]->get_netpoint(), nic, {sg4::LinkInRoute(pcilink)});
  }
...
static sg4::NetZone* create_node(const sg4::NetZone* parent_zone, unsigned long id, bool with_gpus, bool with_nvme)
{
  ...
  add_cpus_and_nic(node_zone, with_gpus, with_nvme);
  node_zone->seal();
